...
namespace Ardutils
{
	...
	constexpr UFixedPoint<Integer, Fraction> Floor(const UFixedPoint<Integer, Fraction> & value)
	{
		...
		return OutputType::FromInternal(static_cast<InternalType>(value.GetInternal() & ~IdentityMask<Fraction>::Value));
	}
	...
	constexpr UFixedPoint<Integer, Fraction> Ceil(const UFixedPoint<Integer, Fraction> & value)
	{
		using OutputType = UFixedPoint<Integer, Fraction>;
		using InternalType = typename OutputType::InternalType;
		return OutputType::FromInternal(static_cast<InternalType>((value.GetInternal() & ~IdentityMask<Fraction>::Value) + (1 << Fraction)));
	}
	...
	constexpr UFixedPoint<Integer, Fraction> Round(const UFixedPoint<Integer, Fraction> & value)
	{
		return (value.GetInternal() & MsbMask<Fraction>::Value) != 0 ? Ceil(value) : Floor(value);
	}
	...
	constexpr FixedPoint<Integer, Fraction> Round(const FixedPoint<Integer, Fraction> & value)
	{
		return
			(
			(value.GetInternal() & MsbMask<Fraction>::Value) != 0 &&
				(Signbit(value) ? (value.GetInternal() & IdentityMask<Fraction - 1>::Value) != 0 : true)
				)
			? Ceil(value)
			: Floor(value);
	}
	...
	constexpr bool Signbit(const FixedPoint<Integer, Fraction> & value)
	{
		return (value.GetInternal() & MsbMask<FixedPoint<Integer, Fraction>::InternalSize>::Value) != 0;
	}
}